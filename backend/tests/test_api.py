...
def test_polygon_minute_aggregates_falls_back_to_history_when_live_is_stale(db_engine, auth_headers):
    TestingSessionLocal = sessionmaker(bind=db_engine)
    ...
    try:
        with TestingSessionLocal() as db:
            db.add_all(
                [
                    ...
                    PolygonMinuteAggregateLive(
                        ticker="SIRI",
                        minute_ts=datetime(2026, 4, 8, 19, 59, 0, tzinfo=timezone.utc),
                        open=9.0,
                        high=9.05,
                        low=8.95,
                        close=9.01,
                    ...
                    PolygonMinuteAggregate(
                        ticker="SIRI",
                        minute_ts=datetime(2026, 4, 9, 13, 30, 0, tzinfo=timezone.utc),
                        open=9.50,
                        high=9.60,
                        low=9.50,
                        close=9.60,
        ...
        assert response.status_code == 200
        body = response.json()
        assert body["trade_date"] == "2026-04-09"
        assert body["source"] == "history"
        assert body["is_stale"] is False
        assert body["total"] == 1
        assert len(body["items"]) == 1
        assert body["items"][0]["ticker"] == "SIRI"
        assert body["items"][0]["minute_ts"].startswith("2026-04-09T13:30:00")
    finally:
        app.dependency_overrides.pop(get_db, None)
